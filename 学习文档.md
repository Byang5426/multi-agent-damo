...
系统通过 OpenRouter 接入多个开源模型，按职责分层选用：

| 层级 | 当前模型 | 选型原则 |
|------|---------|----------|
| Gateway 路由层 | `meta-llama/llama-3.1-8b-instruct` | 追求稳定输出结构化 JSON，小模型降低延迟和成本（~1.7s） |
| PM Agent 编排层 | `mistralai/mistral-small-3.2-24b-instruct` | 优先任务拆解能力和指令遵循（~2.1s） |
| Worker 执行层 | `mistralai/mistral-small-3.2-24b-instruct` | 优先专业能力（代码、分析、测试），质量与速度平衡 |

> 模型通过 `.env` 中 `OPENAI_API_MODEL`（PM/Worker 共用）和 `GATEWAY_MODEL`（路由专用）配置。
...
通过 LLM（轻量模型，当前配置: `llama-3.1-8b-instruct`）对用户请求进行意图分类，输出结构化 JSON：
...
`DEFAULT_PROMPTS` 字典定义了系统内置的 7 个 Prompt（设计参考 agency-agents 社区 Agent 库）：

| Prompt ID | Agent | 角色 | 设计参考 |
|-----------|-------|------|----------|
| `system-gateway` | Gateway | 意图分类提示词 | — |
| `system-pm-decompose` | PM | 项目拆解提示词 | project-manager-senior |
| `system-pm-review` | PM | 验收评审提示词 | project-manager-senior |
| `system-pm-failure` | PM | 失败处理提示词 | project-manager-senior |
| `system-analyst` | Analyst | 需求分析 | software-architect |
| `system-coder` | Coder | 编码实现 | backend-architect |
| `system-tester` | Tester | 测试验收 | test-results-analyzer |
...
| `POST` | `/api/v1/chat/stream` | SSE 流式入口：实时推送执行进度事件 |
| `POST` | `/api/v1/test-llm` | 管理端：测试 LLM 连接可用性 |
...
系统提供 `POST /api/v1/chat/stream` 端点，通过 Server-Sent Events (SSE) 实时推送任务执行进度：

#### 事件类型

| 事件类型 | 说明 | 数据字段 |
|----------|------|----------|
| `gateway` | Gateway 路由决策 | `route`, `reason`, `suggested_worker` |
| `task_start` | Worker 开始执行任务 | `task_index`, `task_title`, `worker` |
| `task_done` | Worker 执行完成 | `task_index`, `status`, `summary` |
| `pm_review` | PM 验收决策 | `approved`, `reason` |
| `error` | 执行错误 | `message` |
| `done` | 全部完成 | `result`（完整 ChatResponse） |

#### 前端集成

```javascript
const eventSource = new EventSource('/api/v1/chat/stream');
// 或使用 fetch + ReadableStream POST 方式
eventSource.onmessage = (event) => {
  const data = JSON.parse(event.data);
  switch (data.type) {
    case 'gateway':    // 更新路由信息
    case 'task_start': // 显示执行中动画
    case 'task_done':  // 更新任务状态
    case 'pm_review':  // 显示验收结果
    case 'done':       // 关闭连接，显示最终结果
  }
};
```
...
- 基于 `asyncio.Queue` + `StreamingResponse` 的异步事件发射模式
- `run_workflow_streaming()` 在每个关键节点发射事件到队列
- datetime 等不可序列化类型通过 `_json_safe()` 辅助函数转为 ISO 格式字符串
